...
const Result = (props) => {
  ...
  if (currentFunction === "add") {
    finalyResult = add(A, parseInt(props.date.valueB), M);
  }
  if (currentFunction === "inverse") {
    finalyResult = inverse(A, M);
  }
  if (currentFunction === "subtract") {
    finalyResult = subtract(A, parseInt(props.date.valueB), M);
  }
  if (currentFunction === "multiply") {
    finalyResult = multiply(A, parseInt(props.date.valueB), M);
  }
  if (currentFunction === "power") {
    finalyResult = power(A, parseInt(props.date.valueB), M);
  }
  if (currentFunction === "division") {
    finalyResult = division(A, parseInt(props.date.valueB), M);
  }
  if (currentFunction === "mod_inversion") {
    finalyResult = modInverse(A, M);
  }
  if (currentFunction === "isPrimeInput") {
    finalyResult = isPrimeFermat(N) ? "просте" : "не просте";
  }
  if (currentFunction === "generatePrime") {
    finalyResult = generateRandomPrime(P);
  }
  return (
